        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (Array.isArray(response.data.data)) {
                    ...
                    console.error('Extracted teeth data is not an array:', response.data.data);
                    setAllExtractedTeeth([]); // Set empty array as fallback
                }
            } else {
                console.error('Eroare la preluarea extractiilor');
                setAllExtractedTeeth([]); // Set empty array on error
            }
        } catch (error) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (Array.isArray(response.data.data)) {
                    ...
                    console.error('Tooth problems data is not an array:', response.data.data);
                    setAllToothProblems([]); // Set empty array as fallback
                }
            } else {
                console.error('Eroare la preluarea interventiilor dintelui');
                setAllToothProblems([]); // Set empty array on error
            }
        } catch (error) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                if (Array.isArray(response.data.data)) {
                    ...
                    console.error('Teeth problems data is not an array:', response.data.data);
                    setTeethPtoblems([]); // Set empty array as fallback
                }
            } else {
                console.error('Eroare la preluarea interventiilor dintelui');
                setTeethPtoblems([]); // Set empty array on error
            }
        } catch (error) {
            console.error(`Eroare la preluarea intervențiilor asupra dintelui ${teethId}`, error);
            setTeethPtoblems([]); // Set empty array on error
        ...
        try {
            ...
            if (response.status === 200) {
                if (Array.isArray(response.data.data)) {
                    ...
                    console.error('Teeth history data is not an array:', response.data.data);
                    setTeethHistory([]); // Set empty array as fallback
                }
            } else {
                console.error('Eroare la preluarea interventiilor dintelui');
                setTeethHistory([]); // Set empty array on error
            }
        } catch (error) {
            console.error(`Eroare la preluarea intervențiilor asupra dintelui ${teethId}`, error);
            setTeethHistory([]); // Set empty array on error
                    ...
                    <div className={style["headPart"]}>
                        <div className={`${style["switch"]} flex items-center gap-2`}>
                            <Switch
                                ...
                                id="dentition-switch"
                            />
                            <Label htmlFor="dentition-switch">Dentiție temporară</Label>
                        </div>
                    </div>
                    {selectedTeeth === null ?
                        (
